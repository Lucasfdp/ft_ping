...
static int	finalise_flags(int ac, char **av, t_flags *flags)
{
	...
	if (flags->bsd_flood && !flags->flood)
		return (fprintf(stderr,
				"ft_ping: --bsd-flood only means anything with -f\n"), 0);
	if (flags->quiet)
	{
...
int	parse_info(int ac, char **av, t_flags *flags)
{
	...
	tmp = 0;
	d_tmp = 0;
	/* Long options first: getopt() understands single letters only, so they
	   are lifted out of argv before it ever sees the vector. */
	ac = extract_long_opts(ac, av, flags);
	if (ac < 0)
		return (0);
	/* Silence getopt's own message: every rejection below prints its own,
	   ...
	   The LEADING COLON in the optstring is what makes a missing argument
	   distinguishable from an unknown option: with it, getopt returns ':'
	   for "-s with nothing after it" and reserves '?' for options that do
	   not exist. Without it both arrive as '?' and the two cases cannot be
	   told apart - which is also what would make a bare -? impossible to
	   recognise as a request for help. */
	opterr = 0;
	while ((opt = getopt(ac, av, ":fl:i:m:oQqrp:S:s:T:t:v")) != -1)
	{
		switch (opt)
		{
			...
			case '?':
				/* getopt reports both "unknown option" and, without the
				   leading ':' in the optstring, "missing argument" as '?'.
				   The leading ':' separates them, so reaching here means
				   genuinely unknown - and if what the user typed WAS '?',
				   they asked for help rather than made a mistake. */
				if (optopt == '?')
