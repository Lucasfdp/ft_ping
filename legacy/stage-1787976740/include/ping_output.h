...
/// @brief Reports a reply whose checksum does not match its contents.
///
/// Goes to stderr, and is printed whatever the display flags say: a
/// corrupted packet is a fault in the network, not a ping result, and
/// silently dropping it would make the loss it causes inexplicable.
///
/// @param src Source address of the corrupted packet, as printable text.
void	print_checksum_mismatch(const char *src);

/* ---- src/output/ping_print_usage.c -------------------------------- */

/// @brief Prints the full usage block on stdout, as -? asks for.
void	print_usage(void);

/// @brief Prints the one-line "Try 'ft_ping -?'" pointer on stderr.
