...
/* ---- src/cli/ping_parse_long.c ------------------------------------ */

/// @brief Lifts every recognised `--long` option out of argv.
///
/// Must run before getopt(), which understands single letters only. argv is
/// compacted in place, so getopt() sees a vector containing nothing but
/// short options and operands.
///
