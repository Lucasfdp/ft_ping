...
# The character -f uses to erase a dot when its reply lands. Naming it
# keeps the flood assertions readable, since a literal \b in a pattern is
...
## Attaches an informational line to a PASSING test.
##
## For results that are correct but worth seeing - an achieved rate well
## below nominal, say, which means the host's timer is coarse rather than
## that anything is wrong. Notes never fail a test.
note() { CUR_NOTES+=("$1"); }
...
expect_le() {   # expect_le <actual> <max> <what>
	...
	if [ "${#CUR_ERRORS[@]}" -eq 0 ]; then
		...
		for e in "${CUR_NOTES[@]:-}"; do
			[ -n "$e" ] && printf '        %snote:%s %s\n' "$C_YEL" "$C_DIM" "$e$C_RST"
		done
...
# These assert a RATIO, not an absolute packet count, and that is deliberate.
#
# -f paces at 10 ms, so the nominal rate is 100 packets/second. Whether the
# machine actually achieves it depends on the host's timer resolution, not on
# this program: poll(2) with a 10 ms timeout returns after ~10 ms on real
# hardware, but inside a VM or under emulation the hypervisor's timer slack
# can stretch that to 20 ms or more, halving the achieved rate through no
# fault of the code. An absolute floor would therefore be testing the machine.
#
# What IS the program's responsibility is that -f floods - that it sends
# dramatically more than the default 1-second interval would, measured on the
# same machine in the same window. That ratio holds everywhere.

## Sends for `secs` seconds with and without -f, and reports both counts.
## Sets FLOOD_N and BASE_N.
flood_vs_default() {
	local secs="$1"
	run_t $((secs + 8)) -t "$secs" -q "$HOST"
	BASE_N=$(sent_count)
	run_t $((secs + 8)) -t "$secs" -q -f "$HOST"
	FLOOD_N=$(sent_count)
}

begin "-f floods far faster than the default interval"
skip_if $NO_LIVE "needs root"
if [ -z "$CUR_SKIP" ]; then
	flood_vs_default 2
	CUR_CMD="$SUDO $BIN -t 2 -q -f $HOST   (compared against $BASE_N without -f)"
	expect_ge "$FLOOD_N" "$(( BASE_N * 10 ))" \
		"packets in a 2s flood, against $BASE_N at the default interval"
	# A floor low enough that no plausible host trips it, but high enough to
	# catch -f being ignored outright.
	expect_ge "$FLOOD_N" 20 "packets in a 2s flood (sanity floor)"
	# 200 is nominal for 2 seconds at 10 ms. Well under it is a coarse host
	# timer, which is worth seeing but is not a defect.
	if [ "$FLOOD_N" -lt 150 ]; then
		note "achieved $((FLOOD_N / 2))/s against a nominal 100/s - this host's poll() timer is coarse (common in a VM or under emulation), not a fault in ft_ping"
	fi
fi
expect_rc 0
end
...
# The marker is the dot/backspace PAIR, not a run of dots. On loopback every
# reply arrives before the next request goes out, so each dot is erased
# immediately and the stream reads ".\b.\b.\b" - four consecutive dots never
# appear. Matching a bare "." would be worse still: every IP address and
# every RTT figure contains one, so it would pass with -f absent.
skip_if $NO_LIVE "needs root"
run_t 10 -t 2 -f "$HOST"
expect_match "\.$BS"
end

begin "--bsd-flood without -f is refused"
run --bsd-flood "$HOST"
expect_rc 1
expect_match "only means anything with -f"
end

begin "an unrecognised long option is refused"
run --no-such-option "$HOST"
expect_rc 1
expect_match "unrecognized option '--no-such-option'"
expect_match "Try 'ft_ping -\?'"
end

begin "--help is a synonym for -?"
run --help
expect_rc 0
expect_match "^Usage: ft_ping"
end

begin "-- still marks the end of options"
# Long options are lifted out of argv before getopt runs, so the pre-pass
# has to honour the end-of-options marker the same way getopt does.
skip_if $NO_LIVE "needs root"
run_t 8 -t 2 -q -- "$HOST"
expect_rc 0
expect_match "^PING $HOST"
...
begin "--bsd-flood sends far more than the timer alone"
# Opt-in BSD behaviour: a packet per reply, on top of the 100/s floor. The
# rate becomes bound by the round-trip time, so on loopback it is orders of
# magnitude above the timer-only default rather than a few percent.
skip_if $NO_LIVE "needs root"
if [ -z "$CUR_SKIP" ]; then
	run_t 10 -t 2 -q -f "$HOST"
	_timer=$(sent_count)
	run_t 10 -t 2 -q -f --bsd-flood "$HOST"
	_bsd=$(sent_count)
	CUR_CMD="$SUDO $BIN -t 2 -q -f --bsd-flood $HOST   (vs $_timer with the timer alone)"
	expect_ge "$_bsd" "$(( _timer * 5 ))" \
		"packets with --bsd-flood, against $_timer from the timer alone"
	note "$_timer/2s timer-paced vs $_bsd/2s reply-paced"
fi
expect_rc 0
end

begin "plain -f stays timer-paced, matching inetutils-2.0"
# The default must NOT send on reply: inetutils ping_run() receives in the
# readable branch and sends in the timeout branch, and they are exclusive.
# 2s at the 10ms floor is ~200 packets; anything near a reply-paced rate
# would be thousands, so a generous ceiling still catches a regression.
skip_if $NO_LIVE "needs root"
run_t 10 -t 2 -q -f "$HOST"
expect_rc 0
expect_le "$(sent_count)" 1000 "packets in a 2s timer-paced flood"
end
...
if [ -z "$CUR_SKIP" ]; then
	...
	CUR_CMD="$SUDO $BIN -t 2 -q -f $HOST   (compared against $BASE_N without -f)"
	expect_ge "$FLOOD_N" "$(( BASE_N * 10 ))" \
		"packets in a 2s quiet flood, against $BASE_N at the default interval"
fi
expect_rc 0
expect_absent "icmp_seq="
# A backspace never appears in ordinary output, so its absence is exactly
# "the flood display was suppressed".
expect_absent "$BS"
end
