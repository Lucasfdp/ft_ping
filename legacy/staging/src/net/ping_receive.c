...
/// @brief Re-runs the checksum over a received ICMP message and compares it
///        against the value the sender put in the header.
///
/// The checksum is a two-ended contract: the sender computes it over a
/// message whose checksum field reads zero, so the receiver has to
/// reproduce exactly that state to verify it. Save the field, zero it,
/// recompute, restore - the restore matters because the ICMP-error path
/// reads the same buffer afterwards.
///
/// Do not assume the kernel already did this. A raw socket exists precisely
/// to see traffic a cooked socket would have filtered, and a packet
/// corrupted in flight is exactly the case this catches.
///
/// @param reply      The ICMP header, at recvbuf + ihl.
/// @param icmp_bytes Bytes of ICMP received, i.e. n - ihl.
/// @return Non-zero when the checksum matches.
...
int	ping_receive(int sockfd, const t_ping_ctx *ctx, t_ping_stats *stats)
{
	...
	while (1)
	{
		...
		{
			/* Corrupted in flight. Report it and drop it: everything after
			   this point would be reading fields we have just proven we
			   cannot trust. */
			inet_ntop(AF_INET, &ip->ip_src, src, sizeof src);
