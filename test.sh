...
# The optstring has no leading ':', so getopt reports this as '?' rather
# than ':'. What matters is that it is refused, not which of the two.
run "$HOST" -s
expect_rc 1
expect_match "(unknown option|needs an argument)"
end
...
expect_match "^round-trip min/avg/max/mdev = [0-9.]+/[0-9.]+/[0-9.]+/[0-9.]+ ms$"
end
